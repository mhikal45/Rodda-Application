...
class ResultAdapter(options: FirestoreRecyclerOptions<ResultModel>) :
    FirestoreRecyclerAdapter<ResultModel, ResultAdapter.ResultsViewHolder>(options) {
    ...
    inner class ResultsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(result: ResultModel) {
            ...
            binding.tvByName.text = result.nama
